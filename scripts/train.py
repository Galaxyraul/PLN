from transformers import AutoModelForSequenceClassification, AutoTokenizer,DataCollatorWithPadding,TrainingArguments, Trainer,set_seed,EarlyStoppingCallback,AutoConfig
...
model_config = AutoConfig.from_pretrained(config['model']['path'])
model_config.hidden_dropout_prob = config['model']['hd']
model_config.attention_probs_dropout_prob = config['model']['ad']

def preprocess(examples):
    return tokenizer(examples[config['text']], padding=True, truncation=True)
model = AutoModelForSequenceClassification.from_config(model_config,num_labels=config['label']['n_classes'])
...
if config['tokenized']:
    ...
    data = load_train_data(config['paths']['data'],config['label']['target'],config['eval_pct'])
...
early_stopping_callback = EarlyStoppingCallback(
    early_stopping_patience=5,         # Stop after 3 evaluation rounds without improvement
