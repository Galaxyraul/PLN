...
model = AutoModelForSequenceClassification.from_pretrained(config['model']['path'],num_labels=config['label']['n_classes'])
model.hidden_dropout_prob = config['model']['hd']
model.attention_probs_dropout_prob = config['model']['ad']
...
trainer = Trainer(
    ...
    callbacks=[early_stopping_callback] if config['model']['early'] else []
)
